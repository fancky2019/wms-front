<template>
  <div>
    <el-dialog      v-dialogDrag  :append-to-body="true" :title="$t('action.receipt')"   width="80%"  :visible.sync="dialogVisibleinner" :close-on-click-modal="false" >
      ...
      <el-row>
        ...
        <el-col   :span="6">
          <el-form-item :label="getDisplayName('xStatus')" prop="xStatus"> 
            <!-- :disabled="operation?true:disabled" -->
      ...
      <el-row>
        ...
        <el-col  :span="6">
          <el-form-item :label="getDisplayName('packageUnitId')" prop="packageUnitId">
            <el-select  v-model="currProptery.packageUnitId"     filterable>
        ...
        <el-col  :span="6">
          <el-form-item :label="getDisplayName('comments')" prop="comments">
            <el-input v-model="currProptery.comments" />
          </el-form-item>
        </el-col>
            ...
            <el-row>
        <el-col   :span="6">
          <el-form-item :label="getDisplayName('receiptOrderId')" prop="receiptOrderId">
            <el-input v-model="currProptery.receiptOrderId" />
          </el-form-item>
        </el-col>
 
        <el-col   :span="6">
          <el-form-item :label="getDisplayName('receiptOrderItemId')" prop="receiptOrderItemId">
            <el-input v-model="currProptery.receiptOrderItemId" />
            <!-- this.currProptery.receiptOrderItemId = this.CurrentEntity.id -->
          </el-form-item>
        </el-col>
        <el-col  :span="6">
          <el-form-item :label="getDisplayName('pallet')" prop="pallet">
            <el-input v-model="currProptery.pallet" />
          </el-form-item>
        </el-col>
        <el-col  :span="6">
          <el-form-item :label="getDisplayName('carton')" prop="carton">
            <el-input v-model="currProptery.carton" />
          </el-form-item>
        </el-col>
      ...
      <el-row>
        <el-col   :span="6">
          <el-form-item :label="getDisplayName('serialNo')" prop="serialNo">
            <el-input v-model="currProptery.serialNo" />
          </el-form-item>
        </el-col>
 
        <!-- <el-col   :span="6">
          <el-form-item :label="getDisplayName('locationId')" prop="locationId" >
            <el-input v-model="currProptery.locationCode" />
          </el-form-item>
        </el-col> -->
        <el-col  :span="6">
          <el-form-item :label="getDisplayName('locationCode')" prop="locationCode">
            <!-- <el-input v-model="currProptery.locationCode" /> -->
            <el-select v-model="currProptery.locationId" filterable :disabled="disabled"  @change="chagneLocate">
                <el-option v-for="item in $store.state.my.locations" :key="item.id" :value=String(item.id) :label="item.xCode"> </el-option>
            </el-select>

          </el-form-item>
        </el-col>
        <el-col  :span="6">
          <el-form-item :label="getDisplayName('receivedSmallUnitQuantity')" prop="receivedSmallUnitQuantity">
            <el-input v-model="currProptery.receivedSmallUnitQuantity" />
          </el-form-item>
        </el-col>
      ...
      <el-row>
        <!-- <el-col   :span="6">
          <el-form-item :label="getDisplayName('isCreateMovedoc')" prop="isCreateMovedoc"> 
            <el-select v-model="currProptery.isCreateMovedoc" placeholder="请选择">
              <el-option
        ...
        <el-col   :span="6">
          <el-form-item :label="getDisplayName('isWeipan')" prop="isWeipan"> 
            <el-select v-model="currProptery.isWeipan" placeholder="请选择">
              <el-option
                v-for="item in optionsChoose"
                :key="item.value"
                :label="item.label"
        ...
        <!-- <el-col  :span="6">
          <el-form-item :label="getDisplayName('inventoryId')" prop="inventoryId">
            <el-input v-model="currProptery.inventoryId" />
          </el-form-item>
        </el-col>
        <el-col  :span="6">
          <el-form-item :label="getDisplayName('inventoryDetailId')" prop="inventoryDetailId">
            <el-input v-model="currProptery.inventoryDetailId" />
          </el-form-item>
        </el-col>  -->
      </el-row>
      <el-row>
        <el-col  :span="6">
          <el-form-item :label="getDisplayName('receivedPkgQuantity')" prop="receivedPkgQuantity">
            <el-input v-model="currProptery.receivedPkgQuantity" />
          </el-form-item>
        </el-col> 
        <el-col  :span="6">
      ...
      <el-row>
		...
		<div slot="footer" class="dialog-footer">
			<!-- :loading="editLoading" -->
			<el-button :size="size" @click.native="dialogVisibleinner = false">{{$t('action.cancel')}}</el-button>
...
import { CreateReceivedRecordDto  } from '@/libs/createDto'
...
export default {
   ...
   computed: {
    currProptery () { 
      return this.CurrentEntity ||  CreateReceivedRecordDto()
    }, 
    currMaterial(){
   ...
   data () {
    return {
      ...
      dialogMaterial:false,
        ...
        dialogVisibleinner: false, // 新增编辑界面是否显示
        roles: [],   
        materialData: [],
        packageUnits:[],
        loading:false,
   ...
   methods: {
    chagneLocate(value){
      console.log("货物的位置哎-->",value)
      console.log("货物的位置哎22-->",this.$store.state.my.locations)
     let obj =  this.$store.state.my.locations.find(function(i){
        return i.id ==value
      })
    this.currProptery.locationCode = obj.xCode
      console.log("货物的位置哎22344-->",obj)
    ...
    findPage() { 
	  ...
	  submitForm() {
            ...
            //     }
            //   } 
            //    this.$http.putAsync(`/app/ReceivedRecord/Update?id=${params.id}`, params).then(data!=null?data:'').catch(error=>{})  
            ...
            //   this.editLoading = false
              ...
              this.$http.postAsync('/app/ReceivedRecord/Create ', params).then(data!=null?data:'').catch(error=>{})
