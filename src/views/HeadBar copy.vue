<template>
  <div class="headbar" :style="{'background':themeColor}" :class="collapse?'position-collapse-left':'position-left'">
    ...
    <!-- 导航菜单 -->
    <span class="navbar">
      <el-menu :default-active="activeIndex" class="el-menu-vertical-demo" :background-color="themeColor" text-color="#fff"
        active-text-color="#ffd04b" mode="horizontal" @select="selectNavBar()">
        <el-menu-item index="1" @click="$router.push('/').catch(err => {})">首页</el-menu-item>
        <el-menu-item index="2" v-popover:interface>
          <li style="color:#fff;">工具栏</li>
          <el-popover ref="interface" placement="bottom-start" trigger="click">
            ...
            wcs</div>
            <div class="lang-item" @click="openWindow('http://localhost:8083/')">servers</div>
          </el-popover>
        </el-menu-item>
      </el-menu>
    </span>
      ...
      if (user) {
        ...
        this.$api.user.findByName(params).then((res) => {
          if (res.httpCode == 200) {
            ...
            this.user.avatar = require("@/assets/logo-min1.jpg")
          }
        })
      }
