    ...
    <el-dialog :title="operation?$t('action.add') : $t('edit')" width="35%"  :visible.sync="dialogVisible" :close-on-click-modal="false" >
		<el-form :model="dataForm" label-width="80px" :rules="dataFormRules" ref="form" :size="size"
			...
			<!-- <el-form-item :label="getDisplayName('allocatRelationId')" prop="allocatRelationId">
				<el-select v-model="dataForm.allocatRelationId" placeholder="请选择" style="width:100%">
			...
			<el-form-item :label="getDisplayName('shipmentRuleId')" prop="shipmentRuleId">
				<el-select v-model="dataForm.shipmentRuleId" placeholder="请选择" style="width:100%">
                 <el-option v-for="(item, k) in $store.state.my.basicStrategies" :key="k"  :value="item"></el-option>
              </el-select>
			</el-form-item>
			...
			<el-form-item :label="getDisplayName('class1')" prop="class1">
				 <el-select v-model="dataForm.class1" placeholder="请选择" style="width:100%">
...
export default {
   ...
   methods: {
    // 获取语言
	getDisplayName (key) {
	   //console.log(this.$store.state.my.warehouses)
				...
				if (valid) {
					this.$confirm(this.$t('confirmSubmission'), this.$t('presentation'), {}).then(() => {
						...
						if (res!=undefined) {
							...
							this.$http.putAsync(`/app/Conveyor/Update?id=${params.id}`, params).then(data!=null?data:'').catch(error=>{})
						  ...
						  let data = res => {
                           ...
                           this.$http.postAsync('/app/Conveyor/Create', params).then(data!=null?data:'').catch(error=>{})
						   this.dialogVisible = false
						}
					})
				}
