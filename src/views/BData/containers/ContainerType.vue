...
<!-- <ContainerTypeManager ref="manager" @on-close="$refs.pages.refresh()" :EntityData="currentRow"    @findPage="findPage"/> -->
...
// import ContainerTypeManager from './ContainerTypeManager'
...
export default {
  ...
  data() {
    return { 
      ...
      columns: [
        ...
        {
          title: this.getDisplayName('agvType'),
          key: 'agvType',
          minWidth: 150
        },
        {
  ...
  methods: {
    ...
    async search (condition) {  
      ...
      let ret = await this.$http.postAsync("/app/Pallet/GetList", param)  
    ...
    findPage: function (data) {
      ...
      this.$http.postAsync("/app/Pallet/GetList", condition).then((res) => {
        if (res.data.result) {
    ...
    // 新增
    handleAdd() {
      this.currentRow = CreateContainerDto()
    ...
    // 删除
	  handleDelete: function (data) {
		 //console.log("删除的数据显示---》",data )
			this.$http.deleteAsync('/app/Pallet/Delete',data.params[0]).then(data!=null?data.callback:'')
		},
    // 导出
    exportExcelUser: function () {
      ...
      this.$http.postAsync('/app/Pallet/GetListExcel', params).then((res) => {
