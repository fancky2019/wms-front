  ...
  <div class="page-container">
    ...
    <header-column :columns="columns" highSearch     @on-search='handleSearch'   ></header-column>
     <div style="display:flex;align-items:center;margin-left:10px">
       ...
       <kt-button icon="fa fa-plus" :label="$t('action.associatedMaterial')" perms="sys:dict:add" type="primary"  @click="addAssociatedMaterial " />
    </div>
	</div>
    ...
    >
  	...
  	<!--新增编辑界面-->
	<AreaMangerVue ref="manager" @on-close="$refs.pages.refresh()" :EntityData="currentRow" @findPage="findPage"/>
  <AreaMaterialVue ref="material" @on-close="$refs.pages.refresh()"  :AreaId="areaId"  />
  </div>
</template>
...
import AreaMangerVue from './AreaManger.vue'
import AreaMaterialVue from './AreaMaterial.vue'
...
export default {
    ...
    dataFormRules: {
        ...
        },
        {
          ...
          key: 'xStatus',
          minWidth: 130,
          items: this.$Enum.WmsArwa.map1,
          render: (h, params) => {
            return h('span', this.$Enum.WmsArwa.map1[params.row.xStatus] || '')
          }
        },
        {
    ...
    async search (condition) {  
      ...
      let ret = await this.$http.postAsync("/app/Area/GetList", param)  
      ...
      let params = {
	    ...
	    this.$http.postAsync("/app/Area/GetList",params).then((res) => {
    ...
    selectChange(selections,row){
     this.areaId = selections.selections[0].id
    },
    // 点击旋转当前行当前行
    rowCurrentRow(row,index) {
      this.areaId = row.index.id
    },
    addAssociatedMaterial(){
     this.$refs.material.dialogVisible = true
    },
    // 显示新增界面
		handleAdd: function () {
    ...
    handleEdit(params) {
	  ...
	  handleDelete: function (data) {
		  this.$http.deleteAsync('/app/Area/Delete',data.params[0]).then(data!=null?data.callback:'').catch(error=>{})
		},
		// 时间格式化
