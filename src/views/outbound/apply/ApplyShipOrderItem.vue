<template>
<el-dialog :title="$t('edit')" width="80%"  :visible.sync="dialogVisible" :close-on-click-modal="false" >
<el-form ref="form" :model="currModel" label-width="95px" >
    ...
    <el-form-item> 
    <kt-table
      :data="currModel.applyShipOrderItems"
      :columns="columns" 
...
export default {
  ...
  methods: {
    ...
    findPage: function (data) {
      ...
      if (this.currModelID) {
        ...
        if (res.data.result) {
         ...
         this.currModel.applyShipOrderItems = res.data.data.items         
        }
      }).catch(error=>{})
      }
    },
	...
	handleDelete: function (data) { 
	  this.$http.deleteAsync('/app/ApplyShipOrderItem/Delete',data.params[0]).then(data!=null?data.callback:'').catch(error=>{})
	},
  }, 
};
</script>
