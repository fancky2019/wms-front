<template>
  <div class="page-container">
    ...
    <!--  -->
    <ShipPickOrderItemManger ref="ShipPickOrderItem" @on-close="$refs.pages.refresh()" :CurrentEntity="currentRow"   @findPage="findPage"   />
  </div>
</template>
<script>
...
import ShipPickOrderItemManger from './ShipPickOrderItemManger'
...
export default {
  ...
  components: {
    ...
    HeaderColumn,
    KtButton,
    ShipPickOrderItemManger
  },
  computed: {
    currModel () { 
      return this.EntityItemData.shipPickOrderItems
    },
  },
  data() {
    return {  
  ...
  methods: {
    ...
    async search (condition) {  
      ...
      // 还需要最外层的基础物料的id 
      let ret = await this.$http.postAsync("/app/ShipPickOrderItem/GetList", param).catch(error=>{})  
    ...
    findPage: function (data) {
      ...
      if (this.currModelID) {
        this.$http.postAsync("/app/ShipPickOrderItem/GetList", condition).then((res) => {
        if (res.data.result) {
          // 用于界面更新
            this.currModel.splice(0,this.currModel.length)
    ...
    //  下载
    handleDownLoad(){ 
      this.$http.getAsync('/app/ShipPickOrderItem/GetCreateManyExcel',{responseType:'blob'}).then((res)=>{
      }).catch(error=>{})
    },
    // 新增
    handleAdd() {
	...
	handleDelete: function (data) { 
	  this.$http.deleteAsync('/app/ShipPickOrderItem/Delete',data.params[0]).then(data!=null?data.callback:'').catch(error=>{})
	},
  }, 
};
</script>
