<template>
  <div class="page-container">
    ...
    <!--  -->
    <InventoryItem ref="InventoryItem" @on-close="$refs.pages.refresh()" :CurrentEntity="currentRow"/>
  </div>
</template>
   ...
   // 获取语言
    ...
    handleSearch (condition) {
     ...
     async search (condition) {  
      ...
      let ret = await this.$http.postAsync("/app/Inventory/GetList", param).catch(error=>{})  
	...
	findPage: function (data) {
      ...
      let params = {
	    ...
	    this.$http.postAsync("/app/Inventory/GetList",params).then((res) => {
          if(res!=undefined){
    ...
    rowCurrentRow(row){
     ...
     handleExpandChild(data){
      ...
      let len = data.expandData.inventoryItems.length
      if(len>0) {
         this.$set(data.expandData,"tableExpandData", data.expandData.inventoryItems) 
      }
    },
	// 时间格式化
