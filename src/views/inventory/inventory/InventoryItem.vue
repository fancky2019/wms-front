<template>
<el-dialog :title="$t('InventoryItemDetail')" width="80%"  :visible.sync="dialogVisible" :close-on-click-modal="false" >
    <!--表格内容栏 @findPage="findPage"--> 
    <kt-table
      :data="currModel"
      :columns="columns" 
...
export default {
  ...
  computed: {
    currModel () { 
      return this.CurrentEntity.inventoryItems[0].inventoryItemDetails   || CreateCountingInventoryItemDto()
    },
  },
  data() {
    return {  
      ...
      columns:[
        ...
        // {
        //   title:  getPropertyDisplayName1('inventoryItemId'),
        //   key: 'inventoryItemId',
        //   minWidth: 150
        // },
        {
  ...
  methods: {
    ...
    //   }
    //   if (this.currModelID) {
    //     this.$http.postAsync("/app/InventoryItemDetail/GetList", condition).then((res) => {
    //     if (res.data.result) {
    //       console.log(res.data.data.items)
    //      this.currModel.inventoryItems = res.data.data.items         
    //     }
    //   }).catch(error=>{})
    //   }
    // },
